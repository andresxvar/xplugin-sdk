...
/* $Id: CPluginMgr.cpp,v 1.12 2006/03/04 22:33:29 cybermind Exp $ */
...
int CPluginMgr::LoadPlugin(const char* file, int iscmd) {
	...
	if (!p->LoadQuery(file, iscmd)) {
		ENG_SYSCALL(G_PRINT, vaf("[QMM] ERROR: CPluginMgr::LoadPlugin(\"%s\"): Unable to load plugin due to previous errors\n", file));
		delete p;
		return 0;
	}
	ENG_SYSCALL(G_PRINT, vaf("[QMM] CPluginMgr::LoadPlugin(\"%s\"): Successfully queried plugin \"%s\"\n", file, p->PluginInfo()->name));

	if (!p->Attach(g_EngineMgr->SysCall(), s_plugin_vmmain, get_pluginfuncs(), iscmd)) {
		ENG_SYSCALL(G_PRINT, vaf("[QMM] CPluginMgr::LoadPlugin(\"%s\"): QMM_Attach() returned 0 for plugin \"%s\"\n", file, p->PluginInfo()->name));
		p->Unload(0);
		delete p;
		return 0;
	}
	ENG_SYSCALL(G_PRINT, vaf("[QMM] CPluginMgr::LoadPlugin(\"%s\"): Successfully attached plugin \"%s\"\n", file, p->PluginInfo()->name));
...
int CPluginMgr::CallvmMain(int cmd, int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9, int arg10, int arg11) {
	...
	while (p) {
		...
		case QMM_UNUSED:
			ENG_SYSCALL(G_PRINT, vaf("[QMM] WARNING: CPluginMgr::CallvmMain(%s): Plugin \"%s\" did not set result flag\n", MOD_MSGNAME(cmd), p->data()->PluginInfo()->name));
			break;

			//error: show error
		case QMM_ERROR:
			ENG_SYSCALL(G_PRINT, vaf("[QMM] ERROR: CPluginMgr::CallvmMain(%s): Plugin \"%s\" resulted in ERROR\n", MOD_MSGNAME(cmd), p->data()->PluginInfo()->name));
			break;
		...
		default:
			ENG_SYSCALL(G_PRINT, vaf("[QMM] ERROR: CPluginMgr::CallvmMain(%s): Plugin \"%s\" set unknown result flag \"%d\"\n", MOD_MSGNAME(cmd), p->data()->PluginInfo()->name, p->data()->Result()));
			break;
		}
...
int CPluginMgr::Callsyscall(int cmd, int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9, int arg10, int arg11, int arg12) {
	...
	while (p) {
		...
		switch (p->data()->Result()) {
			//unchanged: show warning
		case QMM_UNUSED:
			ENG_SYSCALL(G_PRINT, vaf("[QMM] WARNING: CPluginMgr::Callsyscall(%s): Plugin \"%s\" did not set result flag\n", ENG_MSGNAME(cmd), p->data()->PluginInfo()->name));
			break;

			//error: show error
		case QMM_ERROR:
			ENG_SYSCALL(G_PRINT, vaf("[QMM] ERROR: CPluginMgr::Callsyscall(%s): Plugin \"%s\" resulted in ERROR\n", ENG_MSGNAME(cmd), p->data()->PluginInfo()->name));
			break;
		...
		default:
			ENG_SYSCALL(G_PRINT, vaf("[QMM] ERROR: CPluginMgr::Callsyscall(%s): Plugin \"%s\" set unknown result flag \"%d\"\n", ENG_MSGNAME(cmd), p->data()->PluginInfo()->name, p->data()->Result()));
			break;
		}
