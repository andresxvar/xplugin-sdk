...
_inline void VectorMA( const vec3_t veca, float scale, const vec3_t vecb, vec3_t vecc) {
	vecc[0] = veca[0] + scale*vecb[0];
	vecc[1] = veca[1] + scale*vecb[1];
	vecc[2] = veca[2] + scale*vecb[2];
}

_inline vec_t DotProduct( const vec3_t v1, const vec3_t v2 ) {
	return v1[0]*v2[0] + v1[1]*v2[1] + v1[2]*v2[2];
}

_inline void CrossProduct( const vec3_t v1, const vec3_t v2, vec3_t cross ) {
	cross[0] = v1[1]*v2[2] - v1[2]*v2[1];
	cross[1] = v1[2]*v2[0] - v1[0]*v2[2];
	cross[2] = v1[0]*v2[1] - v1[1]*v2[0];
}

_inline void VectorSubtract( const vec3_t veca, const vec3_t vecb, vec3_t o ) {
	o[0] = veca[0]-vecb[0];
	o[1] = veca[1]-vecb[1];
	o[2] = veca[2]-vecb[2];
}

_inline void VectorAdd( const vec3_t veca, const vec3_t vecb, vec3_t o ) {
	o[0] = veca[0]+vecb[0];
	o[1] = veca[1]+vecb[1];
	o[2] = veca[2]+vecb[2];
}

_inline void VectorCopy( const vec3_t in, vec3_t out ) {
	out[0] = in[0];
	out[1] = in[1];
	out[2] = in[2];
}

_inline void VectorScale( const vec3_t i, vec_t scale, vec3_t o ) {
	o[0] = i[0]*scale;
	o[1] = i[1]*scale;
	o[2] = i[2]*scale;
}
...
_inline int VectorCompare( const vec3_t v1, const vec3_t v2 ) {
...
//NOTE: less precise
_inline int VectorCompare2( const vec3_t v1, const vec3_t v2 ) {
...
_inline vec_t VectorLength( const vec3_t v ) {
	return (vec_t)sqrt (v[0]*v[0] + v[1]*v[1] + v[2]*v[2]);
}

_inline vec_t VectorLengthSquared( const vec3_t v ) {
	return (v[0]*v[0] + v[1]*v[1] + v[2]*v[2]);
}

_inline vec_t Distance( const vec3_t p1, const vec3_t p2 ) {
	vec3_t	v;
...
_inline vec_t DistanceSquared( const vec3_t p1, const vec3_t p2 ) {
	vec3_t	v;
...
_inline void VectorNormalizeFast( vec3_t v )
{
	float ilength;
...
_inline void VectorRotate( vec3_t in, vec3_t matrix[3], vec3_t out )
{
...
//
//char *Info_ValueForKey( const char *s, const char *key ); lmd_plugins
